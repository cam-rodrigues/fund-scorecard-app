...
def extract_fund_performance(pdf_file):
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            for line in lines:
                # Fund name or ticker (e.g., "XYZ Fund Class A (XYZAX)")
                if re.search(r"[A-Z]{4,6}X", line):
                    fund_name = line.strip()
                    continue

                # Extract numerical values from metrics line
                if fund_name and re.search(r"[-+]?\d+\.\d+", line):
                    ...
                    if len(numbers) >= 6:
                        ...
                        fund_name = None  # Reset for next fund
...
# === Streamlit App ===
def run():
    st.set_page_config(page_title="Fund Performance Comparison", layout="wide")
