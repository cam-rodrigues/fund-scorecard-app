        ...
        if not df.empty:
            ...
            with st.expander("Download Results"):
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    ...
                    # === Add FidSync note ===
                    worksheet.write("A1", f"Generated by FidSync on {datetime.now().strftime('%Y-%m-%d %I:%M %p')}", note_format)
                
                    # === Format headers ===
                    for col_num, value in enumerate(df.columns.values):
                        worksheet.write(2, col_num, value, header_format)
                
                    # === Format body and autofit ===
                    for row_idx, row in enumerate(df.itertuples(index=False), start=3):
                        for col_idx, value in enumerate(row):
                            fmt = metric_format if isinstance(value, str) and value in ("Pass", "Review") else text_format
