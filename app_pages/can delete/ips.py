...
def process_page1(text):
    ...
    st.write(f"- Total Options: {st.session_state.get('total_options')}")
    st.write(f"- Prepared For: {st.session_state.get('prepared_for')}")
...
def step3_process_scorecard(pdf, start_page, declared_total):
    ...
    for p in pdf.pages[start_page-1:]:
        ...
        # keep scanning until you hit next major break (heuristic: blank after scoreboard)
        if "Fund Scorecard" in txt or pages:
            pages.append(txt)
        else:
            break
    lines = "\n".join(pages).splitlines()

    # drop explanatory block if present
    ...
    for i, line in enumerate(lines):
        # match "<Metric Name> Pass/Review <explanation>"
        m = re.match(r"^(Manager Tenure|Excess Performance \(3Yr\)|Excess Performance \(5Yr\)|Peer Return Rank \(3Yr\)|Peer Return Rank \(5Yr\)|Expense Ratio Rank|Sharpe Ratio Rank \(3Yr\)|Sharpe Ratio Rank \(5Yr\)|R-Squared \(3Yr\)|R-Squared \(5Yr\)|Sortino Ratio Rank \(3Yr\)|Sortino Ratio Rank \(5Yr\)|Tracking Error Rank \(3Yr\)|Tracking Error Rank \(5Yr\))\s+(Pass|Review)\s*(.*)$", line.strip(), flags=re.IGNORECASE)
        if m:
            metric_name = m.group(1).strip()
            status = m.group(2).strip().capitalize()
            explanation = m.group(3).strip()

            # If Manager Tenure appears, backfill new fund name from previous non-empty line
            if metric_name.lower() == "manager tenure":
                if current_name and current_metrics:
                    fund_blocks.append({"Fund Name": current_name, "Metrics": current_metrics})
                # find previous non-empty line as fund name
                prev = ""
                for j in range(i-1, -1, -1):
                    if lines[j].strip():
                        ...
                        break
                # clean name
                cleaned = re.sub(r"(Meets Watchlist Criteria\.?|Watchlist Criteria\.?)", "", prev, flags=re.IGNORECASE).strip()
                current_name = cleaned
                current_metrics = []

            if current_name:
                # store metric with its raw info including status and explanation
                info = f"{status}" + (f": {explanation}" if explanation else "")
                current_metrics.append({"Metric": metric_name, "Info": info})
        else:
            # attach continuation to previous metric if it exists
            if current_metrics:
                last = current_metrics[-1]
                # avoid accidentally including a new fund header
                if not re.match(r"^(Manager Tenure|Excess Performance|Peer Return Rank|Expense Ratio Rank|Sharpe Ratio Rank|R-Squared|Sortino Ratio Rank|Tracking Error Rank)", line, flags=re.IGNORECASE):
                    last["Info"] += " " + line.strip()
...
def step4_ips_screen():
    ...
    for b in st.session_state.get("fund_blocks", []):
        ...
        # Manager Tenure ≥3
        info = next((m["Info"] for m in b["Metrics"] if m["Metric"].lower() == "manager tenure"), "")
        ...
        for metric in IPS[1:]:
            # match by startswith to handle spacing/casing
            m_obj = next((x for x in b["Metrics"] if x["Metric"].lower().startswith(metric.split()[0].lower())), None)
            info = m_obj["Info"] if m_obj else ""
...
def build_and_display_scorecard_tables():
    ...
    tickers_raw = st.session_state.get("tickers", {}) or st.session_state.get("fund_performance_data", {})
    if isinstance(tickers_raw, list):
        tickers_map = {d.get("Fund Scorecard Name", ""): d.get("Ticker", "") for d in tickers_raw}
    else:
        tickers_map = tickers_raw
