...
# === Load secrets ===
secrets = st.secrets
HUGGINGFACE_API_KEY = secrets.get("huggingface", {}).get("api_key", "")
TOGETHER_API_KEY = secrets.get("together", {}).get("api_key", "")
...
model_choice = st.selectbox("Choose Model", ["FinGPT (Hugging Face)", "Together AI"])
...
"""
            ...
            if model_choice.startswith("FinGPT") and HUGGINGFACE_API_KEY:
                ...
                if response.status_code == 200:
                    terms = response.json()
                    text_out = terms[0]["generated_text"] if isinstance(terms, list) and "generated_text" in terms[0] else ""
                else:
                    text_out = ""
...
# Rename everything above into a function:
def main():
    import streamlit as st
    import pdfplumber
    import requests
