...
#-------------------------------------------------------------------------------------------
        ...
        factsheet_matches = [
            {
                "Fund Name": row["Matched Fund Name"],
                "Ticker": row["Matched Ticker"],
                "Page Number": row["Page #"] - 1  # 0-indexed
            }
            for row in st.session_state.get("fund_factsheets_data", [])
            if row.get("Matched") == "✅"
        ]
        
        if not factsheet_matches:
            st.error("❌ No matched factsheets found to extract Risk-Adjusted Returns.")
        else:
            risk_returns_data = []
        
            with pdfplumber.open(uploaded_file) as pdf:
                for match in factsheet_matches:
                    ...
                    for i, line in enumerate(lines):
                        if "risk-adjusted returns" in line.lower():
                            table_lines = lines[i+1:i+8]
                            for row in table_lines:
                                row = row.strip()
                                for label in ["Sharpe", "Information", "Sortino"]:
                                    if label.lower() in row.lower():
                                        ...
                                        if len(parts) >= 5:
                                            ...
                                            found_table = True
                            break  # Only process first Risk-Adjusted block
            ...
            if not risk_returns_data:
                ...
                for fund in risk_returns_data:
                    st.markdown(f"### {fund['Fund Name']} ({fund['Ticker']})")
                    df = pd.DataFrame({
                        "Metric": ["Sharpe Ratio", "Information Ratio", "Sortino Ratio"],
                        "1 Yr": [
                            fund["Sharpe Ratio"].get("1 Yr", ""),
                            fund["Information Ratio"].get("1 Yr", ""),
                            fund["Sortino Ratio"].get("1 Yr", "")
                        ],
                        "3 Yrs": [
                            fund["Sharpe Ratio"].get("3 Yrs", ""),
                            fund["Information Ratio"].get("3 Yrs", ""),
                            fund["Sortino Ratio"].get("3 Yrs", "")
                        ],
                        "5 Yrs": [
                            fund["Sharpe Ratio"].get("5 Yrs", ""),
                            fund["Information Ratio"].get("5 Yrs", ""),
                            fund["Sortino Ratio"].get("5 Yrs", "")
                        ],
                        "10 Yrs": [
                            fund["Sharpe Ratio"].get("10 Yrs", ""),
                            fund["Information Ratio"].get("10 Yrs", ""),
                            fund["Sortino Ratio"].get("10 Yrs", "")
                        ]
                    })
                    st.dataframe(df, use_container_width=True)
